...
def train(cfg, env, eval_env, agent, buffer):
    ...
    """
    print(f"--> Start Off-Policy Training (VDN/IDQN) | Seed: {cfg.seed}")
    ...
    # Using agent's init_hidden (B, N, L, H)
    ...
    while total_steps < cfg.max_steps:
        ...
        env_dones = np.any(dones, axis=1) | np.any(truncated, axis=1)
        if np.any(env_dones):
            # Mask hidden state (B, N, L, H) with (B,)
