    ...
    while total_steps < cfg.max_steps:
        with torch.no_grad():
            while not buffer.is_full():
                ...
                dones = np.array(dones)          # (E, N)
                truncated = np.array(truncated)  # (E, N) or (E,)
                
                if truncated.ndim == 1:
                    truncated = truncated[:, None] & np.ones_like(dones, dtype=bool)
                ...
                for i in range(cfg.num_envs):
                    ...
                    if np.any(dones[i]) or np.any(truncated[i]):
                        ...
                        if i_episode % getattr(cfg, 'eval_freq', 1000) == 0:
                            ...
                            pbar.write(f" [Seed {cfg.seed}] Steps {total_steps} | Episodes {i_episode}: Eval Reward = {avg_reward:.2f}")
